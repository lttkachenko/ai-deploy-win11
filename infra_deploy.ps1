...
# Enforce clean target directory layout matrix (DOS-7 / DOS-9 Compliance)
$infrastructureDirs = @("bin", "conf", "log", "models", "context")
foreach ($dir in $infrastructureDirs) {
  $targetPath = Join-Path $aiRoot $dir
...
$resourcePools = @("artifacts", "insights", "prompts", "roles", "skills", "tools", "user")
...
if (Test-Path $sourceContextRoot) {
  ...
  foreach ($pool in $resourcePools) {
    # Match layout components ignoring case mutations
...
# 4. Phase 1: Deploying LM Studio Engine
...
# 6. Phase 3: Host Network Topology Alignment
